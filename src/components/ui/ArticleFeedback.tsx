...
export default function ArticleFeedback({ slug }: { slug: string }) {
  ...
  const submit = async () => {
    ...
    setSending(true)
    try {
      await supabase.from("article_feedback").insert({ slug, rating, comment: comment.trim()||null })
      setSent(true)
      toast.success("Merci pour votre retour ! ⭐")
    } catch { setSent(true); toast.success("Merci ! ⭐") }
  ...
  if (sent) return (
    <div style={{ marginTop:24, padding:"24px", background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.3)", borderRadius:14, textAlign:"center" }}>
      ...
      <div style={{ marginTop:12, display:"flex", justifyContent:"center", gap:4 }}>
        {[1,2,3,4,5].map(i => <span key={i} style={{ fontSize:24, color:i<=rating?"#f59e0b":"#334155" }}>★</span>)}
      </div>
    </div>
  )
  ...
  return (
    <div style={{ marginTop:24, padding:"20px 24px", background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:14 }}>
      <div style={{ fontSize:15, fontWeight:700, color:"var(--text-1)", marginBottom:4 }}>📝 Cet article vous a été utile ?</div>
      <div style={{ fontSize:12, color:"var(--text-3)", marginBottom:16 }}>Votre avis nous aide à améliorer le contenu PMO AI Studio</div>

      {/* Non connecté */}
      {!user && (
        <div style={{ padding:"14px 16px", background:"rgba(123,94,255,0.06)", border:"1px solid rgba(123,94,255,0.2)", borderRadius:10 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <Link href="/auth/register" style={{ padding:"6px 14px", background:"var(--primary)", color:"#fff", borderRadius:7, fontSize:11, fontWeight:700, textDecoration:"none" }}>Créer un compte gratuit →</Link>
            <Link href="/auth/login" style={{ padding:"6px 14px", background:"transparent", border:"1px solid var(--border)", color:"var(--text-2)", borderRadius:7, fontSize:11, textDecoration:"none" }}>Se connecter</Link>
          </div>
        </div>
      )}
      ...
      {/* Email non vérifié */}
      {user && !emailVerified && (
        <div style={{ padding:"14px 16px", background:"rgba(245,158,11,0.08)", border:"1px solid rgba(245,158,11,0.3)", borderRadius:10 }}>
      ...
      {/* Formulaire — connecté + email vérifié */}
      {user && emailVerified && (
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:12 }}>
            ...
            {(hover||rating) > 0 && (
              <span style={{ fontSize:13, fontWeight:600, color:COLORS[hover||rating], marginLeft:6 }}>{LABELS[hover||rating]}</span>
            )}
          </div>
          {rating > 0 && (
            <div style={{ marginBottom:12 }}>
              <textarea value={comment} onChange={e => setComment(e.target.value)}
                placeholder="Un commentaire ? Une suggestion ? (optionnel)" rows={3}
