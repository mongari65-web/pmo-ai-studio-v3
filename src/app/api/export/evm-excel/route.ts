...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><defs><style>text{font-family:Arial,Helvetica,sans-serif}</style></defs><rect width="${W}" height="${H}" fill="#0f172a" rx="8"/><text x="${W/2}" y="28" fill="#f1f5f9" font-size="14" font-weight="bold" text-anchor="middle">Courbe S EVM - ${projectName}</text>${yLines}${xLabels}${cpL}<polyline points="${pts(pvCum)}" fill="none" stroke="#3b82f6" stroke-width="2.5"/><polyline points="${pts(evCum)}" fill="none" stroke="#22c55e" stroke-width="2.5"/><polyline points="${pts(acCum)}" fill="none" stroke="#f59e0b" stroke-width="2.5"/>${pvCum.map((v,i)=>`<circle cx="${padL+i*xStep}" cy="${yS(v)}" r="3" fill="#3b82f6"/>`).join("")}${evCum.map((v,i)=>`<circle cx="${padL+i*xStep}" cy="${yS(v)}" r="3" fill="#22c55e"/>`).join("")}${acCum.map((v,i)=>`<circle cx="${padL+i*xStep}" cy="${yS(v)}" r="3" fill="#f59e0b"/>`).join("")}<rect x="180" y="8" width="10" height="10" fill="#3b82f6"/><text x="194" y="18" fill="#94a3b8" font-size="11">PV</text><rect x="230" y="8" width="10" height="10" fill="#22c55e"/><text x="244" y="18" fill="#94a3b8" font-size="11">EV</text><rect x="280" y="8" width="10" height="10" fill="#f59e0b"/><text x="294" y="18" fill="#94a3b8" font-size="11">AC</text></svg>`
