...
export default function BudgetPage() {
  ...
  const generate = async () => {
    if (!project) return
    setLoading(true)
    toast.info("Génération Budget EVM en cours...")
    try {
      ...
      toast.success("Budget EVM généré — " + newItems.length + " éléments")
    } catch (e: any) {
  ...
  return (
    <AppLayout>
      <ToolLayout
        title="Budget EVM" icon="💰" subtitle="// EARNED VALUE MANAGEMENT"
        history={history}
        onLoadHistory={(e) => {
          loadHistory(e)
        ...
        onGenerate={generate}
        generateLabel="Générer Budget EVM"
        generating={loading}
        projectName={project?.name}
        exportRows={items.map((item, i) => ({ Index: i + 1, ...item }))}
        exportFilename={"budget_" + (project?.name ?? "")}>
        ...
        {items.length === 0 && !loading && (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Cliquez sur "Générer Budget EVM" pour démarrer
            </p>
          </div>
        )}
