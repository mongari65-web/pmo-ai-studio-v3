...
export default function MindmapPage() {
  ...
  const generate = async () => {
    if (!project) return
    setLoading(true)
    toast.info("Génération Mind Map en cours...")
    try {
      ...
      toast.success("Mind Map généré — " + newItems.length + " éléments")
    } catch (e: any) {
  ...
  return (
    <AppLayout>
      <ToolLayout
        title="Mind Map" icon="🧠" subtitle="// VISUALISATION"
        history={history}
        onLoadHistory={(e) => {
          loadHistory(e)
        ...
        onGenerate={generate}
        generateLabel="Générer Mind Map"
        generating={loading}
        projectName={project?.name}
        ...
        {items.length === 0 && !loading && (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Cliquez sur "Générer Mind Map" pour démarrer
            </p>
          </div>
        )}
