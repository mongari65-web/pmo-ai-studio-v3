...
export default function DocumentsPage() {
  ...
  const generate = async () => {
    if (!project) return
    setLoading(true)
    toast.info("Génération Documents en cours...")
    try {
      ...
      toast.success("Documents généré — " + newItems.length + " éléments")
    } catch (e: any) {
  ...
  return (
    <AppLayout>
      <ToolLayout
        title="Documents" icon="📄" subtitle="// BIBLIOTHÈQUE DOCUMENTAIRE"
        history={history}
        onLoadHistory={(e) => {
          loadHistory(e)
        ...
        onGenerate={generate}
        generateLabel="Générer Documents"
        generating={loading}
        projectName={project?.name}
        exportRows={items.map((item, i) => ({ Index: i + 1, ...item }))}
        exportFilename={"documents_" + (project?.name ?? "")}>
        ...
        {items.length === 0 && !loading && (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Cliquez sur "Générer Documents" pour démarrer
            </p>
          </div>
        )}
