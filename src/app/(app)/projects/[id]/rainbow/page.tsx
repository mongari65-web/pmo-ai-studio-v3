  ...
  return (
    <AppLayout>
      <div style={{ padding:"16px 20px 32px", background:"var(--bg)", minHeight:"100%" }}>
        ...
        {/* RAINBOW CONTENT */}
        <div id="rainbow-content" style={{ background:"#f8fafc",borderRadius:16,overflow:"hidden",boxShadow:"0 8px 40px rgba(0,0,0,0.12)",border:"1px solid #e2e8f0" }}>
          ...
          <div style={{ background:"linear-gradient(90deg,rgba(30,64,175,0.95) 0%,rgba(109,40,217,0.85) 50%,rgba(220,38,38,0.7) 100%)",padding:"28px 32px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:20 }}>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:10,color:"#64748b",textTransform:"uppercase",letterSpacing:"2px",marginBottom:8 }}>Rapport de Gouvernance — PMO AI Studio</div>
              ...
              <div style={{ display:"flex",gap:16,flexWrap:"wrap" }}>
                {[
                  ...
                  <span key={i} style={{ fontSize:12,color:"#1e293b",display:"flex",alignItems:"center",gap:4 }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10,alignItems:"center" }}>
              <div style={{ textAlign:"center",background:"#ffffff",borderRadius:16,padding:"16px 24px",boxShadow:"0 4px 20px rgba(0,0,0,0.1)",border:`2px solid ${ragColor}` }}>
                <div style={{ fontSize:52,fontWeight:900,color:ragColor,lineHeight:1 }}>{score}</div>
                <div style={{ fontSize:11,color:"#475569",marginTop:4 }}>Score Santé</div>
                <div style={{ fontSize:18,fontWeight:800,color:ragColor,marginTop:6 }}>{rag==="Vert"?"🟢":rag==="Ambre"?"🟡":"🔴"} {rag}</div>
              </div>
              <div style={{ textAlign:"center",background:"rgba(0,0,0,0.2)",borderRadius:10,padding:"8px 16px",fontSize:14,color:"#0f172a",fontWeight:600 }}>
                {meteo}
              </div>
            </div>
          </div>
          ...
          <div style={{ padding:"24px 32px",display:"flex",flexDirection:"column",gap:18 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr",gap:16 }}>
              ...
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"18px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>📋 Avancement du projet</div>
                <div style={{ display:"flex",alignItems:"center",gap:16,marginBottom:16 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                      <span style={{ fontSize:13,color:"#1e293b" }}>Progression globale</span>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10 }}>
                  {[
                    ...
                    <div key={i} style={{ textAlign:"center",background:"#f1f5f9",borderRadius:8,padding:"10px 8px" }}>
              ...
              {/* Budget jauge */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"18px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>💰 Budget</div>
                {bac>0 ? (
                  <>
                    <div style={{ display:"flex",justifyContent:"center",marginBottom:12 }}>
                      <svg width="120" height="120" viewBox="0 0 120 120">
                        ...
                        <text x="60" y="72" fill="#64748b" fontSize="10" textAnchor="middle">consommé</text>
                      </svg>
                    </div>
                    <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                      {[["BAC",fmt(bac),"#64748b"],["AC",fmt(ac),"#f59e0b"],["EAC",fmt(eac),pctBudget>100?"#ef4444":"#22c55e"]].map(([l,v,c]:any,i)=>(
                ...
                ) : <p style={{ fontSize:12,color:"#94a3b8",textAlign:"center",padding:"20px 0" }}>Budget non défini</p>}
              </div>
            </div>
            ...
            {bac>0 && (
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>📊 Indicateurs EVM — {MONTHS[cp]}</div>
                <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:10 }}>
                  {[
                    ["BAC",fmt(bac),"Budget total","#1e293b"],
                    ["EV",fmt(ev),"Valeur acquise","#3b82f6"],
                    ["AC",fmt(ac),"Coût réel","#f59e0b"],
                    ["CPI",cpi?.toFixed(2)??"-","Perf. coût",cpi!==null?(cpi>=1?"#22c55e":cpi>=0.9?"#f59e0b":"#ef4444"):"#64748b"],
                    ["SPI",spi?.toFixed(2)??"-","Perf. délai",spi!==null?(spi>=1?"#22c55e":spi>=0.9?"#f59e0b":"#ef4444"):"#64748b"],
                    ...
                    <div key={i} style={{ textAlign:"center",background:"#f1f5f9",borderRadius:10,padding:"12px 8px",border:`1px solid ${c}22` }}>
                      <div style={{ fontSize:9,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4 }}>{l}</div>
                      <div style={{ fontSize:16,fontWeight:900,color:c,lineHeight:1 }}>{v}</div>
                      <div style={{ fontSize:9,color:"#94a3b8",marginTop:3 }}>{s}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
              ...
              {bac>0 && (
                <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                  <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10 }}>📈 Courbe S EVM</div>
                  <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
                    {[0,0.25,0.5,0.75,1].map(r=>(
                      <line key={r} x1={pL} y1={pT+cH*(1-r)} x2={W-pR} y2={pT+cH*(1-r)} stroke="#f1f5f9" strokeWidth="1"/>
                    ))}
                    ...
                    {MONTHS.map((m,i)=>(
                      <text key={m} x={pL+i*xSt} y={H-5} fill="#94a3b8" fontSize="8" textAnchor="middle">{m}</text>
                    ))}
                  </svg>
              ...
              {/* RAID */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>⚠️ RAID — Résumé</div>
                <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12 }}>
                  {[[raidItems.length,"Total","#475569"],[raidCrit.length,"Critiques","#ef4444"],[raidOpen.length,"Ouverts","#f59e0b"],[raidResolu.length,"Résolus","#22c55e"]].map(([v,l,c]:any,i)=>(
                    <div key={i} style={{ textAlign:"center",background:"#f1f5f9",borderRadius:8,padding:"10px 6px" }}>
                ...
                {raidCrit.slice(0,4).map((r:any,i:number)=>(
                  <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#fef2f2",border:"1px solid rgba(239,68,68,0.15)",marginBottom:6 }}>
                    <AlertTriangle size={12} color="#ef4444" style={{ flexShrink:0 }}/>
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontSize:12,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{r.title}</div>
                      <div style={{ fontSize:10,color:"#94a3b8" }}>{r.category} · {r.owner??"-"}</div>
                    </div>
                  </div>
                ))}
                {raidCrit.length===0 && raidOpen.length>0 && raidOpen.slice(0,3).map((r:any,i:number)=>(
                  <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#fffbeb",marginBottom:6 }}>
                    <AlertTriangle size={12} color="#f59e0b" style={{ flexShrink:0 }}/>
                    <span style={{ fontSize:12,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{r.title}</span>
                  </div>
                ))}
                {raidItems.length===0 && <p style={{ fontSize:12,color:"#94a3b8",textAlign:"center",padding:"16px 0" }}>Aucun élément RAID</p>}
              </div>
            </div>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
              {/* Jalons */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>🏁 Jalons clés</div>
                <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12 }}>
                  {[[jalonsDone.length,"Atteints","#22c55e"],[jalonsNext.length,"À venir","#3b82f6"],[jalonsLate.length,"En retard","#ef4444"]].map(([v,l,c]:any,i)=>(
                    <div key={i} style={{ textAlign:"center",background:"#f1f5f9",borderRadius:8,padding:"10px" }}>
                      <div style={{ fontSize:22,fontWeight:900,color:c }}>{v}</div>
                      <div style={{ fontSize:10,color:"#94a3b8" }}>{l}</div>
                    </div>
                  ))}
                ...
                {jalonsLate.slice(0,2).map((j:any,i:number)=>(
                  <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#fef2f2",marginBottom:5 }}>
                    <AlertTriangle size={11} color="#ef4444" style={{ flexShrink:0 }}/>
                    <span style={{ fontSize:12,color:"#1e293b",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{j.name}</span>
                ...
                {jalonsNext.slice(0,4).map((j:any,i:number)=>{
                  ...
                  return (
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#eff6ff",marginBottom:5 }}>
                ...
                {jalons.length===0 && <p style={{ fontSize:12,color:"#94a3b8",textAlign:"center",padding:"16px 0" }}>Aucun jalon défini</p>}
              </div>

              {/* OKR */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12 }}>
                  <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px" }}>🎯 OKR — Progression</div>
                  {objectives.length>0 && <span style={{ fontSize:14,fontWeight:900,color:pc(avgOKR) }}>{avgOKR}% moy.</span>}
                </div>
                {objectives.length===0 && <p style={{ fontSize:12,color:"#94a3b8",textAlign:"center",padding:"16px 0" }}>Aucun OKR défini</p>}
                {objectives.slice(0,5).map((obj:any,i:number)=>{
                  const prog=objProg(obj); const pColor=pc(prog)
                  return (
                    <div key={i} style={{ marginBottom:12 }}>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5 }}>
                        <span style={{ fontSize:11,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"80%",fontWeight:500 }}>{obj.title}</span>
                      ...
                      <div style={{ fontSize:10,color:"#94a3b8",marginTop:2 }}>{obj.category} · {obj.quarter}</div>
                    </div>
                  )
                })}
              </div>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
              ...
              {/* Décisions récentes */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>📌 Décisions récentes</div>
                {decisions.length===0 && <p style={{ fontSize:12,color:"#94a3b8",textAlign:"center",padding:"16px 0" }}>Aucune décision enregistrée</p>}
                {decisions.map((d:any,i:number)=>(
                  <div key={i} style={{ display:"flex",gap:10,padding:"8px 10px",borderRadius:8,background:"#f8fafc",marginBottom:6,border:"1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontSize:12,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:500 }}>{d.title??d.decision??d.description??"Décision"}</div>
              ...
              {/* Équipe projet */}
              <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px",marginBottom:12 }}>👥 Équipe projet</div>
                {members.length===0 && (
                  <div style={{ textAlign:"center",padding:"16px 0" }}>
                    <Users size={28} color="#cbd5e1" style={{ marginBottom:8 }}/>
                    <p style={{ fontSize:12,color:"#94a3b8" }}>Aucun membre ajouté</p>
                  </div>
                )}
                {/* Chef de projet */}
                {project.chef_de_projet && (
                  <div style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:"#fffbeb",border:"1px solid rgba(245,158,11,0.15)",marginBottom:8 }}>
                    ...
                    <div>
                      <div style={{ fontSize:12,fontWeight:700,color:"#0f172a" }}>{project.chef_de_projet}</div>
                ...
                {members.slice(0,5).map((m:any,i:number)=>{
                  ...
                  return (
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:8,background:"#f8fafc",marginBottom:5 }}>
                      ...
                      <div style={{ flex:1,minWidth:0 }}>
                        <div style={{ fontSize:12,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{m.email}</div>
            ...
            <div style={{ background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:"16px 20px" }}>
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10 }}>
                <div style={{ fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:"1px" }}>💬 Commentaire de gouvernance</div>
                <button className="no-print" onClick={()=>editComment?saveComment():setEditComment(true)}
                  style={{ fontSize:11,padding:"4px 12px",borderRadius:6,border:"1px solid rgba(255,255,255,0.15)",background:"#e2e8f0",color:"#1e293b",cursor:"pointer" }}>
                  {editComment?"💾 Sauvegarder":"✏️ Modifier"}
                </button>
              </div>
              {editComment ? (
                <textarea value={comment} onChange={e=>setComment(e.target.value)}
                  style={{ width:"100%",background:"#f1f5f9",border:"1px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"12px",color:"#0f172a",fontSize:13,lineHeight:1.6,resize:"vertical",minHeight:80,outline:"none",fontFamily:"inherit",boxSizing:"border-box" }}
                  placeholder="Saisissez un commentaire pour la gouvernance (points d'attention, décisions, orientations)..."/>
              ) : (
                <p style={{ fontSize:13,color:comment?"#1e293b":"#cbd5e1",lineHeight:1.6,margin:0,fontStyle:comment?"normal":"italic" }}>
