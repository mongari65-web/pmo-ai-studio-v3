...
export default function WorkpackagesPage() {
  ...
  const generate = async () => {
    if (!project) return
    setLoading(true)
    toast.info("Génération Work Packages en cours...")
    try {
      ...
      toast.success("Work Packages généré — " + newItems.length + " éléments")
    } catch (e: any) {
  ...
  return (
    <AppLayout>
      <ToolLayout
        ...
        onGenerate={generate}
        generateLabel="Générer Work Packages"
        generating={loading}
        projectName={project?.name}
        exportRows={items.map((item, i) => ({ Index: i + 1, ...item }))}
        exportFilename={"workpackages_" + (project?.name ?? "")}>
        ...
        {items.length === 0 && !loading && (
          <div className="bg-card border border-dashed border-border rounded-xl p-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Cliquez sur "Générer Work Packages" pour démarrer
            </p>
          </div>
        )}
